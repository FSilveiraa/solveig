...
from tests.test_utils import DEFAULT_CONFIG, RequirementFactory, MockRequirementMixin
...
class TestPluginHookSystem:
    ...
    def test_before_hook_validation_error(self):
        ...
        req = RequirementFactory.create_command_requirement(comment="Test", command="fail this command")
    ...
    def test_before_hook_security_error(self):
        ...
        req = RequirementFactory.create_command_requirement(comment="Test", command="rm -rf /important/data")
    ...
    def test_after_hook_processing_error(self):
        ...
        def failing_processor(config: SolveigConfig, requirement: CommandRequirement, result: CommandResult):
            if result.accepted:
                raise ProcessingError("Post-processing failed")

        req = RequirementFactory.create_command_requirement(comment="Test", command="echo hello")
    ...
    def test_multiple_before_hooks(self):
        ...
        req = RequirementFactory.create_command_requirement(comment="Test", command="echo test")
    ...
    def test_hook_requirement_filtering(self):
        ...
        cmd_req = RequirementFactory.create_command_requirement(comment="Test", command="echo test")
        cmd_req.solve(DEFAULT_CONFIG)
        
        # Test with ReadRequirement  
        read_req = RequirementFactory.create_read_requirement(comment="Test", path="/test/file", only_read_metadata=True)
