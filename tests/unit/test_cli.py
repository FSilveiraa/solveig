...
from tests.utils.mocks import DEFAULT_CONFIG
...
class TestCLIInterfaceLLMResponse:
    ...
    def test_display_llm_response_with_requirements(self):
        ...
        requirements = [
            ReadRequirement(path="/test/file.txt", only_read_metadata=False, comment="Read file"),
            WriteRequirement(path="/test/output.txt", content="content", is_directory=False, comment="Write file"),
            CommandRequirement(command="ls -la", comment="List files")
        ]
        message = LLMMessage(comment="Response with requirements", requirements=requirements)
    ...
    def test_display_llm_response_grouped_requirements(self):
        ...
        requirements = [
            ReadRequirement(path="/test/file1.txt", only_read_metadata=False, comment="Read file 1"),
            ReadRequirement(path="/test/file2.txt", only_read_metadata=True, comment="Read file 2"),
            WriteRequirement(path="/test/output.txt", content="content", is_directory=False, comment="Write file"),
        ]
