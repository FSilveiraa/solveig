...
class TestBashTimestamps:
    ...
    def test_add_bash_timestamps_new_bashrc(self, mock_open_file, mock_exists, mock_read, mock_home, mock_print):
    ...
    @patch('pathlib.Path.read_text')
    @patch('pathlib.Path.exists')
    def test_add_bash_timestamps_already_configured(self, mock_exists, mock_read, mock_home, mock_print):
        ...
        assert result is True
        mock_print.assert_called_with("✓ Bash history timestamps are already configured.")

    @patch('builtins.print')
    @patch('scripts.solveig_init.Path.home')
    @patch('pathlib.Path.read_text')
    @patch('pathlib.Path.exists')
    def test_add_bash_timestamps_exception(self, mock_exists, mock_read, mock_home, mock_print):
...
class TestDependencyCheck:
    ...
    def test_check_dependencies_missing(self, mock_import, mock_print):
        ...
        mock_import.side_effect = [True, ImportError(), True, True, True]  # Second import fails
...
class TestMainFunction:
    ...
    @patch('builtins.print')
    def test_main_success_with_bash_setup(self, mock_print, mock_add_bash, mock_check_deps, 
                                         mock_create_config, mock_check_tools, mock_ask_yes_no):
    ...
    @patch('scripts.solveig_init.check_dependencies')
    @patch('builtins.print')
    def test_main_skip_bash_setup(self, mock_print, mock_check_deps, 
                                 mock_create_config, mock_check_tools, mock_ask_yes_no):
    ...
    @patch('scripts.solveig_init.check_dependencies')
    @patch('builtins.print')
    def test_main_config_directory_failure(self, mock_print, mock_check_deps, mock_create_config):
