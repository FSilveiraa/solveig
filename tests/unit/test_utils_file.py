...
class TestFilesystemValidation:
    ...
    def test_validate_read_access_permission_denied(self, mock_filesystem):
        ...
        mock_filesystem._entries[abs_path].metadata.is_readable = False

        with pytest.raises(PermissionError, match="not readable"):
            Filesystem.validate_read_access("/test/unreadable.txt")

    def test_validate_write_access_success(self, mock_filesystem):
    ...
    def test_validate_write_access_directory_overwrite(self, mock_filesystem):
    ...
    def test_validate_write_access_insufficient_space(self, mock_filesystem):
        """Test write access validation with insufficient disk space."""
        # Try to write more than available space
        huge_size = mock_filesystem.total_size + 1000
    ...
    def test_validate_delete_access_parent_not_writable(self, mock_filesystem):
        ...
        mock_filesystem._entries[parent_path].metadata.is_writable = False
...
class TestFilesystemHelpers:
    ...
    def test_closest_writable_parent_existing_writable(self, mock_filesystem):
        """Test finding closest writable parent for existing writable directory."""
        test_dir = Filesystem.get_absolute_path("/test/dir")
        result = Filesystem._closest_writable_parent(test_dir)
        assert result == test_dir

    def test_closest_writable_parent_nonexistent_path(self, mock_filesystem):
        ...
        result = Filesystem._closest_writable_parent(deep_path)
    ...
    def test_is_readable(self, mock_filesystem):
    ...
    def test_is_writable(self, mock_filesystem):
...
class TestFilesystemHighLevelOperations:
    ...
    def test_write_file_creates_parent_directories(self, mock_filesystem):
        ...
        assert mock_filesystem.exists(Filesystem.get_absolute_path("/test/deep"))
        assert mock_filesystem.exists(Filesystem.get_absolute_path("/test/deep/nested"))
    ...
    def test_write_file_overwrite_mode(self, mock_filesystem):
        ...
        abs_path = Filesystem.get_absolute_path(test_path)
        content = mock_filesystem.read_file(abs_path).content
        assert content == "new content"

    def test_create_directory_new(self, mock_filesystem):
        ...
        assert mock_filesystem.exists(abs_path)
        assert mock_filesystem.is_dir(abs_path)
    ...
    def test_copy_file_operations(self, mock_filesystem):
        ...
        assert mock_filesystem.exists(dest_abs)
        src_content = mock_filesystem.read_file(src_abs).content
        dest_content = mock_filesystem.read_file(dest_abs).content
        assert src_content == dest_content

    def test_copy_directory_operations(self, mock_filesystem):
        ...
        assert mock_filesystem.exists(dest_abs)
        assert mock_filesystem.is_dir(dest_abs)

    def test_move_file_operations(self, mock_filesystem):
        ...
        src_abs = Filesystem.get_absolute_path(src_path)
        original_content = mock_filesystem._mock_read_text(src_abs)
        ...
        dest_abs = Filesystem.get_absolute_path(dest_path)
        assert not mock_filesystem.exists(src_abs)
        assert mock_filesystem.exists(dest_abs)
        assert mock_filesystem.read_file(dest_abs).content == original_content

    def test_delete_file_operations(self, mock_filesystem):
    ...
    def test_delete_directory_operations(self, mock_filesystem):
    ...
    def test_read_file_text_format(self):
        ...
        read_result = Filesystem.read_file(text_path)
        assert read_result.content == "text content"
        assert read_result.encoding == "text"

    def test_read_file_binary_format(self, mock_filesystem):
        ...
        mock_filesystem.mocks.is_text_file.side_effect = [False]  # Return False once
...
class TestFilesystemErrorHandling:
    ...
    def test_move_destination_already_exists(self):
    ...
    def test_write_file_insufficient_space(self, mock_filesystem):
        ...
        try:
            ...
            mock_filesystem.total_size = original_total_size

    def test_write_file_to_parent_not_writable(self, mock_filesystem):
        ...
        mock_filesystem._entries[test_dir].metadata.is_writable = False
    ...
    def test_write_validation_with_minimum_space_requirements(
        ...
        try:
            ...
            mock_filesystem.total_size = 5000  # 5KB
...
class TestMockFilesystemSideEffects:
    ...
    def test_mock_method_side_effects(self, mock_filesystem):
        """Test that mock methods can have side effects set."""

        # Set a side effect on the create_directory mock
        mock_filesystem.mocks.create_directory.side_effect = [
    ...
    def test_mock_exists_side_effect(self, mock_filesystem):
        """Test that exists mock can have side effects."""
        # Set side effect that raises exception
        mock_filesystem.mocks.exists.side_effect = OSError("Disk error")
    ...
    def test_validate_read_access_permission_denied_scenarios(
        ...
        mock_filesystem._entries[parent_dir].metadata.is_readable = False
        ...
        mock_filesystem._entries[file_path].metadata.is_readable = False
...
class TestFilesystemIntegration:
    ...
    def test_create_nested_structure(self, mock_filesystem):
        ...
        assert "# Project" in readme_content.content

        main_content = mock_filesystem.read_file(
            Filesystem.get_absolute_path(f"{base_dir}/src/main.py")
        )
        assert "print('hello')" in main_content.content

    def test_copy_and_modify_workflow(self, mock_filesystem):
        ...
        assert mock_filesystem.exists(src_abs)
        assert mock_filesystem.exists(dest_abs)
    ...
    def test_complex_directory_operations(self, mock_filesystem):
        ...
        assert mock_filesystem._mock_exists(copy_base)
        assert mock_filesystem._mock_is_dir(copy_base)
