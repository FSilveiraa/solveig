...
from solveig.utils.filesystem import Filesystem, Metadata, parse_size_notation_into_bytes
...
class TestFilesystemValidation:
    ...
    def test_validate_write_access_insufficient_space(self, mock_all_file_operations):
        ...
        with pytest.raises(OSError, match="Insufficient disk space"):
            Filesystem.validate_write_access("/test/huge_file.txt", content_size=huge_size)
    
    def test_validate_write_access_existing_file_not_writable(self, mock_all_file_operations):
...
class TestFilesystemHighLevelOperations:
    ...
    def test_get_directory_listing(self, mock_all_file_operations):
        ...
        assert all(isinstance(path, Path) for path in listing)
        
        # Should contain expected files/directories from mock setup
...
class TestFilesystemIntegration:
    ...
    def test_create_nested_structure(self, mock_all_file_operations):
        ...
        assert fs._mock_exists(Filesystem.get_absolute_path(f"{base_dir}/tests/test_main.py"))
        
        # Verify content
        readme_content = fs._mock_read_text(Filesystem.get_absolute_path(f"{base_dir}/README.md"))
        assert "# Project" in readme_content
        
        main_content = fs._mock_read_text(Filesystem.get_absolute_path(f"{base_dir}/src/main.py"))
