...
class TestInstructorClientFactory:
    ...
    def test_get_instructor_client_gemini(self, mock_configure, mock_model, mock_instructor):
    ...
    def test_get_instructor_client_gemini_import_error(self):
        ...
        with patch('builtins.__import__', side_effect=ImportError):
            with pytest.raises(ValueError, match="Google Generative AI client not available"):
                llm.get_instructor_client(
                    api_type=APIType.GEMINI,
                    api_key="test-key",
                    url="https://generativelanguage.googleapis.com/v1beta"
