...
from solveig.plugins.hooks.shellcheck import check_command, is_obviously_dangerous, DANGEROUS_PATTERNS
...
class TestShellcheckPlugin:
    ...
    def test_dangerous_patterns_detection(self):
        ...
        for cmd in safe_commands:
            assert not is_obviously_dangerous(cmd), f"Should not detect '{cmd}' as dangerous"
    ...
    def test_normal_command_passes_validation(self):
        ...
        config = DEFAULT_CONFIG
        req = CommandRequirement(comment="Test normal command", command="echo 'hello world'")
...
class TestShellcheckPluginIntegration:
    ...
    def test_plugin_requirement_filtering(self):
        ...
        with patch('solveig.utils.misc.ask_yes', return_value=False), \
             patch('solveig.utils.file.validate_read_access', side_effect=FileNotFoundError("File not found")):
