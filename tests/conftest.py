...
def mock_filesystem(request):
    ...
    with patch("builtins.open", side_effect=OSError("Cannot use actual file I/O - use utils.file.Filesystem or mark with @pytest.mark.no_file_mocking")):
...
def mock_subprocess(request):
    ...
    with patch("subprocess.run", side_effect=OSError("Cannot run actual processes - use @patch(\"subprocess.run\") or mark with @pytest.mark.no_subprocess_mocking")) as mocked_subprocess:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "no_subprocess_mocking: mark test to skip automatic subprocess mocking"
    )
