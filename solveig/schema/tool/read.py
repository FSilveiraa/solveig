...
from solveig.schema.result.read import ReadResult
    ...
    async with interface.with_group(
        ...
        if metadata.is_directory or metadata_only:
            ...
            if send_metadata:
                ...
                return ToolReturn(
                    ...
                    metadata=ReadResult(accepted=True, path=path, metadata=metadata),
                )
            await interface.display_warning("Rejected")
            return ToolReturn(return_value="User declined to send metadata.")
        ...
        if choice in (0, 1):
            if line_ranges:
                ...
                content_ranges = [(1, line_count, content_str)]
                await interface.display_text_box(
                    content_str,
            ...
            if send_choice == 0:
                ...
                return ToolReturn(
                    return_value=str(BaseSolveigModel._dump_pydantic_field(metadata)),
                    metadata=ReadResult(accepted=False, path=path, metadata=metadata),
                )
            await interface.display_warning("Rejected")
            return ToolReturn(return_value="User declined to send anything.")
