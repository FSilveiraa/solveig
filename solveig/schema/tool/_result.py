"""ToolResult - the raw shape every tool/hook works with before finalization.

Tool functions and `@before`/`@after` hooks all deal in `ToolResult`, never in
`pydantic_ai.messages.ToolReturn` directly. A `Finalizer` (always the
outermost toolset wrapper) is the only place a `ToolResult` gets converted
into a `ToolReturn` - see `solveig/schema/hooks.py`.
"""
