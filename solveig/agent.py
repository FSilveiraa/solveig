...
held separately in a `ClientRef` and reused across turns; only the `Agent`
...
from solveig.llm.api import ClientRef, get_model
...
def build_agent(
    ...
    directly (e.g. `FunctionModel`/`TestModel`), bypassing `client_ref`'s
    ...
    if model is not None:
        ...
        resolved_model = get_model(config.api_type, client_ref.client, config.model)
    return Agent(
        resolved_model,
